...
sudoku :: Board -> Board
sudoku b = fromElements $ head $ sudokuElements [bL] eL where (bL,eL) = toElements b
...
sudokuElements bes (e:empty) = sudokuElements [(e, p):b | b <- bes , p <- validVals b e] empty
...
validVals bes (x,y) = [1..9] \\ [v | ((r,c), v) <- bes, r == x || c == y || (r `div` 3 == x `div` 3 && c `div` 3 == y `div` 3)]
...
toElements b = ([(c,v) | (c,v) <- bes, v /= 0], [c | (c,v) <- bes, v == 0]) where bes = zip [(x,y) | x<-[0..8], y<-[0..8]] $ concat b
...
fromElements bes = [ [v | (_,v) <- xs] | xs <- xss] where xss = groupBy ((==) `on` (fst.fst)) $ sort bes
