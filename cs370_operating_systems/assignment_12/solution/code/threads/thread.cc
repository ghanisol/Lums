...
Thread::Thread(char* threadName)
{
    name = threadName;
    //modified code
    priority = 1;			// Set Default Priority as 1
...
//---------------------------------------------------------------------
Thread::Thread(char* threadName, int p, int j)
{
    join = j;
    name = threadName;
    priority = p;
...
Thread::Yield ()
{
	...
	if (nextThread->getEffectivePriority() > this->getEffectivePriority())
	{
	    List *readyList = scheduler->getReadyList();
	    readyList->SortedInsert((void *)nextThread, nextThread->getEffectivePriority()); 
	    scheduler->Run(this);
	}
	else
	{
...
void Thread::setEffectivePriority(int p)
{
    if (p > 0 && p <= 5)
    {
